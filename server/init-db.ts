...
export async function initializeDatabase() {
  ...
  try {
    ...
    for (const migrationFile of migrationFiles) {
      ...
      console.log(`[DB Init] Running migration: ${path.basename(migrationFile)}`);
        ...
        if (statement.length > 0) {
          try {
            ...
            if (!error.message.includes('Duplicate') && !error.message.includes('already exists')) {
              console.error(`[DB Init] Error executing statement:`, error.message);
              throw error;
            }
          }
        }
      ...
      console.log(`[DB Init] ✓ Completed migration: ${path.basename(migrationFile)}`);
    }
    ...
    console.log(`[DB Init] Created ${(finalTables as any[]).length} tables:`, 
      (finalTables as any[]).map((t: any) => t.table_name || t.TABLE_NAME).join(', '));
