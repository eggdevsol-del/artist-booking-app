...
describe('useChatController', () => {
    ...
    beforeEach(() => {
        ...
        // Default queries
        (trpc.conversations.getById.useQuery as any).mockReturnValue({ data: { id: 1 } });
        (trpc.messages.list.useQuery as any).mockReturnValue({ data: [] });
        (trpc.messages.send.useMutation as any).mockReturnValue({ mutate: vi.fn() });
        ...
        (trpc.conversations.markAsRead.useMutation as any).mockReturnValue({ mutate: vi.fn() });
        (trpc.upload.uploadImage.useMutation as any).mockReturnValue({ mutate: vi.fn() });
        (trpc.appointments.bookProject.useMutation as any).mockReturnValue({ mutate: vi.fn() });
        (trpc.messages.updateMetadata.useMutation as any).mockReturnValue({ mutate: vi.fn() });
        (trpc.conversations.pinConsultation.useMutation as any).mockReturnValue({ mutate: vi.fn() });
    });
    ...
    it('should send a message', () => {
        ...
        (trpc.messages.send.useMutation as any).mockReturnValue({ mutate: mutateMock });
    ...
    it('should handle quick actions', () => {
        ...
        act(() => {
            result.current.handleQuickAction({ actionType: 'send_text', content: 'Quick Reply' });
        });
