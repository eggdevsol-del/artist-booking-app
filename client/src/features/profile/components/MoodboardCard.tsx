...
export function MoodboardCard({ boards, isEditMode, onCreateBoard, onDeleteBoard, onAddImage }: MoodboardCardProps) {
    ...
    if (selectedBoard) {
        return (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="h-full">
                <div className="flex items-center gap-2 mb-4">
                    <Button variant="ghost" size="icon" onClick={() => setSelectedBoardId(null)} className="-ml-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                    ...
                    {selectedBoard.previewImages.map((img, idx) => (
                        <div key={idx} className="aspect-square rounded-xl overflow-hidden bg-muted relative group">
                                ...
                                <div className="w-16 h-16 rounded-lg bg-black/20 overflow-hidden grid grid-cols-2 gap-0.5">
                                    ...
                                    {[0, 1, 2, 3].map(i => (
                                        <div key={i} className="bg-white/5 h-full w-full">
                                            {board.previewImages[i] && <img src={board.previewImages[i]} className="w-full h-full object-cover" />}
                                        </div>
                                    ))}
                                </div>
                                <div>
