...
export function BookingWizard({ isOpen, onClose, conversationId, artistServices, artistSettings, onBookingSuccess, overlayName, overlayId }: BookingWizardProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
            {/* 1. Portal to ensure document-level rendering */}
            <DialogPrimitive.Portal>
                {/* 2. Backdrop: Subtle Blur + Dim */}
                ...
                {/* 3. Modal Shell: Full Screen, No Transforms, Viewport Anchored */}
                                            ...
                                            <SelectableCard
                                                ...
                                                subtitle={
                                                    <div className="flex gap-3 text-xs text-muted-foreground font-mono">
                                                        ...
                                                        <span className={cn("font-bold", (selectedService && selectedService.id === service.id) ? "text-primary" : "text-muted-foreground")}>${service.price}</span>
