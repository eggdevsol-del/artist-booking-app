...
const componentMap = {
    ...
    'Textarea': 'textarea',
    'Badge': 'badge',
    'Dialog': 'dialog', 'DialogContent': 'dialog', 'DialogDescription': 'dialog', 'DialogTitle': 'dialog', 'DialogFooter': 'dialog', 'DialogHeader': 'dialog',
    'Sheet': 'sheet', 'SheetContent': 'sheet', 'SheetDescription': 'sheet', 'SheetTitle': 'sheet', 'SheetHeader': 'sheet',
    'Skeleton': 'skeleton',
    'Card': 'card',
...
filesToFix.forEach(file => {
    ...
    let newContent = content.replace(regex, (match, importsBody) => {
        const imports = importsBody.split(',').map(s => s.trim()).filter(Boolean);
        ...
        return Object.entries(importsByFile).map(([source, contractImports]) => {
            ...
            const uiRoot = path.resolve('f:/booking app/artist-booking-app/client/src/components/ui');
