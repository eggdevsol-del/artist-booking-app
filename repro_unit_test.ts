...
async function runTest() {
    ...
    if (parsed.policies.length !== 2) {
        throw new Error(`Expected 2 enabled policies, got ${parsed.policies.length}`);
    }

    const hasDeposit = parsed.policies.some((p: any) => p.title === "Deposit Policy");
    const hasCancel = parsed.policies.some((p: any) => p.title === "Cancel Policy");
